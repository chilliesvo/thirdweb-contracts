...
const { blockTimestamp, generateMerkleTree, genNumbersASC, parseEthers, hexProof, ZERO_ADDRESS, parseEther, setTime } = require("../utils");
const { setMerkleRoots, importContractABIs, generateInputsFixedWithTokenSingleNotAvailable, WEIGHT_DECIMAL } = require("../osb.utils");
...
describe("Admin Create Project Case", () => {
	beforeEach(async () => {
        //** Get Wallets */
		[deployer, superAdmin, admin, manager, user1, user2, user3, author, crossmint] = await ethers.getSigners();
	...
	it("Create project without token available", async () => {
		...
		await project.connect(admin).publish(
			[token, name, symbol, baseUri, !isPack, isSingle, isFixed, isInstantPayment, royaltyReceiver, royaltyFeeNumerator, minSales, fixedPricePack, maxPricePack, minPricePack, priceDecrementAmtPack, profitShareAdmin, saleStart, saleEnd],
			saleInputs,
			{ value: createProjectFee }
		);
			
    	//** Add winners */  
    	const merkleTree = generateMerkleTree([user1.address, user2.address, user3.address]);
		...
		await expect(() => sale.connect(user1).buy(1, hexProof(merkleTree, user1.address), 1, { value: raisePrices[0] })).to.changeEtherBalances(
			[superAdmin, author, user1],
			[raisePrices[0], 0, raisePrices[0].mul(-1)]
		);

		await expect(() => sale.connect(user2).buy(2, hexProof(merkleTree, user2.address), 1, { value: raisePrices[1] })).to.changeEtherBalances(
			[superAdmin, author, user2],
			[raisePrices[1], 0, raisePrices[1].mul(-1)]
		);

		await expect(() => sale.connect(user3).buy(3, hexProof(merkleTree, user3.address), 1, { value: raisePrices[2] })).to.changeEtherBalances(
			[superAdmin, author, user3],
			[raisePrices[2], 0, raisePrices[2].mul(-1)]
		);
		...
		await project.connect(admin).closeProject(1, [4, 5], false);
    	expect(await tokenSingle.ownerOf(4)).to.equal(admin.address);
    	expect(await tokenSingle.ownerOf(5)).to.equal(admin.address);
  	})
})